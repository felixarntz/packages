...
const ISSUE_WITH_PARENS_REGEX = /\s\((fix(es)?|see) #(\d+)\)/i;
const ISSUE_REGEX = /(fix(es)?|see) #(\d+)/i;
const FILE_ENTRY_REGEX =
  /( .(dist|env|eslint|git|npm|nvm|prettier))|(config.js|.dist|.json|.xml|.yml)|(lint )|(readme)/i;
const ACTIONS_ENTRY_REGEX =
  /(GitHub)|(GitHub action)|(GH action)|(GitHub workflow)|(GH workflow)|(actions\/)|(Plugin Check)/i;
const DEPENDENCY_ENTRY_REGEX =
  /(dependency)|(dependencies)|(versions)|(test)|(tests)/i;
const MERGE_ENTRY_REGEX = /^Merge( remote-tracking)? branch/i;
const VERSION_ENTRY_REGEX = /(^Bump )|(since annotation)/i;
const FIX_ISSUE_REGEX = /Fix(es)? #(\d+)/i;
...
interface ChangelogEntryReference {
  id: string;
...
const findTagConstraints = async (
  octokit: Octokit,
...
const getCommitConstraints = async (
  ...
  await findTagConstraints(octokit, githubRepository, tag, constraints);
  return constraints;
};

const getCommitEntries = (
  _octokit: Octokit,
  githubRepository: GithubRepository,
  commits: Commit[]
) => {
...
const removePrCommitsFromEntries = async (
  octokit: Octokit,
  githubRepository: GithubRepository,
  prNumber: number,
...
const applyPullRequestToEntries = async (
  ...
  const matches = pr.body?.match(FIX_ISSUE_REGEX);
  if (matches) {
    const issueNumber = Number.parseInt(matches[2], 10);
  ...
  await removePrCommitsFromEntries(
    octokit,
    githubRepository,
    pr.number,
    commitShaLookup,
...
const amendCommitEntriesWithPullRequests = async (
  ...
  for await (const prs of prsGenerator) {
    for (const pr of prs) {
      await applyPullRequestToEntries(
        octokit,
        githubRepository,
        pr,
        commitShaLookup,
...
const amendEntriesWithIssues = async (
  ...
  await Promise.all(
    entries.map(async (entry) => {
      const matches = entry.summary.match(ISSUE_WITH_PARENS_REGEX);
      if (matches) {
        const issueNumber = Number.parseInt(matches[3], 10);
      ...
      const matches2 = entry.summary.match(ISSUE_REGEX);
      if (matches2) {
        const issueNumber = Number.parseInt(matches2[3], 10);
        return await amendEntryWithIssue(
          octokit,
          githubRepository,
          entry,
...
const stripEntriesToSkip = (entries: ChangelogEntry[]) =>
  entries.filter((entry) => {
    if (FILE_ENTRY_REGEX.test(entry.summary)) {
      return false;
    }
    if (ACTIONS_ENTRY_REGEX.test(entry.summary)) {
      return false;
    }
    if (DEPENDENCY_ENTRY_REGEX.test(entry.summary)) {
      return false;
    }
    if (MERGE_ENTRY_REGEX.test(entry.summary)) {
      return false;
    }
    if (VERSION_ENTRY_REGEX.test(entry.summary)) {
      return false;
    }
    return true;
  });
